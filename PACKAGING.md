...
ORBIT `0.1.4` is distributed as a signed x64 Windows release. The primary public artifact is the self-contained Xbox Mode setup:

`release/ORBIT-XboxMode-Setup-0.1.4-x64.exe`

This single executable embeds the complete signed AppX, the public ORBIT certificate and the hardened installation script. Releases beginning with `0.1.2-beta.2`, including stable `0.1.4`, use the non-exportable Certum SimplySign cloud key; the historical stable `0.1.1` release remains self-signed. No PFX, PIN, OTP or SimplySign token is stored in the repository, build output or release asset.
...
For the `0.1.4` stable release:

- product and Git tag: `0.1.4` / `v0.1.4`;
- Windows and AppX identity: `0.1.4.0`;
- release sequence: `14`.
