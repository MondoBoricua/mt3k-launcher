...
async function main() {
  ...
  assert.deepEqual(await js('[...document.querySelectorAll("#settings-language-panel button")].map(e => e.textContent)'), ['English', 'Deutsch', 'Español', 'Русский'])
  await js('settingsQA.focus("#settings-language-panel [data-focusable]")')
  await pad(15); await pad(15); await pad(15); await pad(0)
  assert.equal(await js('settingsQA.state().savedLanguage'), 'ru', 'Controller selects and saves Russian')
  assert.equal(await js('document.documentElement.lang'), 'ru')
  assert.ok(await js('document.body.innerText.includes("Настройки")'))
  await pad(1)
  ...
  assert.equal(await js('languageQA.current()'), 'ru', 'Russian survives settings reload')
  await js('languageQA.fail(true); languageQA.change("de")')
  assert.equal(await js('languageQA.current()'), 'ru', 'Save failure restores confirmed language')
  assert.equal(await js('document.documentElement.lang'), 'ru')
  assert.equal(await js('languageQA.notifications().at(-1).messageKey'), 'settings.saveFailed')
  await js('languageQA.fail(false); settingsQA.clearNotifications(); Promise.all([languageQA.change("de"), languageQA.change("en"), languageQA.change("es"), languageQA.change("ru")])')
  assert.deepEqual(await js('languageQA.languageWrites.slice(-4)'), ['de','en','es','ru'])
  assert.equal(await js('settingsQA.state().savedLanguage'), 'ru')
  ...
  if (!process.argv.includes('--keyboard-only')) for (const [width, height, theme, density] of [[1280,720,'midnight','standard'],[1920,1080,'coresense','compact']]) {
    ...
    for (const page of ['appearance','experience','plus','libraries','hardware','updates','system']) {
      ...
      for (const id of headers) {
        ...
        if (['language','sound','hardware-control','orbit-updates'].includes(id)) await capture(`${id}-${width}-ru`)
        await key('Escape')
        assert.equal(await js('settingsQA.state().focused'), id)
      }
        console.log(`Russian ${page}: ${width} x ${height}, ${density} passed`)
    }
    for (const view of ['home','library','store','details','onboarding']) {
      ...
      await capture(`${view}-${width}-ru`)
    }
  }
  win.setContentSize(1280, 720)
  await js(`(() => { const input = document.createElement('input'); input.id='russian-input'; input.setAttribute('data-focusable',''); input.setAttribute('aria-label','Имя'); document.body.append(input); input.focus(); window.dispatchEvent(new CustomEvent('orbit:gamepad-keyboard:open',{ detail: input })); })()`)
  await settle()
  ...
  for (const char of ['ё','й','ц','ъ','э','я','ю']) await keyboardClick(char)
  await keyboardClick('Shift')
  await keyboardClick('Ё')
  assert.equal(await js('document.getElementById("russian-input").value'), 'ёйцъэяюЁ')
  await keyboardClick('Цифры и символы')
  assert.equal(await js('[...document.querySelectorAll("[data-gamepad-keyboard] button")].every(e => { const r=e.getBoundingClientRect(); return r.top>=0 && r.bottom<=innerHeight && r.left>=0 && r.right<=innerWidth })'), true, 'All Russian keyboard keys fit at 720p')
  ...
  await fs.writeFile(path.join(output,'result.json'), JSON.stringify({ passed:true, language:'ru', viewports:[[1280,720],[1920,1080]], entries:1974, checks:['default','legacy','settings','onboarding','keyboard','controller','persistence','save-error','rapid-switch','layout'] },null,2))
  console.log('Russian integration: defaults, all language controls, persistence, failure recovery, rapid switches, 720p/1080p views, Cyrillic keyboard, controller and back navigation passed.')
  win.destroy()
}
