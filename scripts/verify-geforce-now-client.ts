...
assert.match(panel, /window\.api\.geforceNow\.isNativeAvailable\(\)/u)
assert.match(panel, /window\.api\.applications\.launch\(GEFORCE_NOW_APPLICATION_ID\)/u)
...
assert.match(panel, /library\.geforceNow\.mode\.native/u)
...
assert.match(ipcHandlers, /applicationService\.launchGeForceNowGame\(match, mainWindow, /u)
